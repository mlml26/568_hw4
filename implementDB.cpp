...
void query(int id, pugi::xml_node & rst) {
  //  pugi::xml_node rst;
...
void update_add_account(int account_id, float price, float shares) {
  ...
  if(shares >= 0) {//buy order
    ...
    s = "UPDATE POSITIONTB SET AMOUNT = AMOUNT + " + to_string(shares) + " WHERE ACCOUNTID = " + to_string(account_id) + ";";
  }
  work W(*C);
  W.exec(s);
  W.commit();
}
void cancel(int id, pugi::xml_node & answer) {
  cout << "Calling cancel function." << endl;
  //pugi::xml_document res;
  //pugi::xml_node answer = res.append_child("result");
  //pugi::xml_node answer;
  string sql1 = "SELECT * FROM OPENTB WHERE id = " + to_string(id) + ";";
  string sql2 = "SELECT AMOUNT, PRICE, EPOCH FROM CANCELED WHERE id = " + to_string(id) + ";";
  string sql3 = "SELECT * FROM EXECUTED WHERE id = " + to_string(id) + ";";
  work W(*C);
  ...
  update_add_account(account_id, price, shares);
...
void add_executed(int OrderID, string symbol, float amount, float price, time_t epoch, int accountID){
  work W(*C);
  string sql = "INSERT INTO EXECUTED(ID, SYMBOL, AMOUNT, PRICE, EPOCH, ACCOUNTID) VALUES (" + to_string(OrderID) + ", " +
      W.quote(symbol) + ", " + to_string(amount) + ", " + to_string(price) + ", " + to_string(epoch) + ", " + to_string(accountID) + ");";
  W.exec(sql);
  W.commit();
  //execute(sql);
}
...
void change_amount(int ID, int amount){
  string sqlStatement;
  sqlStatement =  "UPDATE POSITIONTB SET amount = amount + " + to_string(amount) + " WHERE ID = " + to_string(ID) + ";";
  execute(sqlStatement);
}
...
void execute_order(int sellOrderID, int buyOrderID, string symbol, int price, float amount){
  ...
  string sql_sellerID = "SELECT ACCOUNTID, AMOUNT FROM OPENTB WHERE id = " + to_string(sellOrderID) + ";";
    ...
    if(buyerPositionID_res.empty()){
      ...
      change_amount(buyerPositionID_res[0][0].as<int>(), amount);
    }
...
void open_order(int id, string symbol, float amount, float price, int accountID){
  work W(*C);
  string sqlStatement = "INSERT INTO OPENTB(ID, SYMBOL, AMOUNT, PRICE, ACCOUNTID) VALUES (" + to_string(id) + ", " + W.quote(symbol) + ", " + to_string(amount) + ", " + to_string(price) + ", " + to_string(accountID) + ");";
  //    work W(*C);
  W.exec(sqlStatement);
  W.commit();
}
int add_order(int account_id, string symbol, float shares, float price) {
 ...
 update_minus_account(account_id, price, shares);
 ++order_id;
 //insert into openTB
